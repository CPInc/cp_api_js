...
var fullURL = function fullURL(path) {
  ...
  var url = _config2.default.config().baseURL + path;
...
var unauthorizedPromise = function unauthorizedPromise() {
  return new Promise(function (resolve) {
    resolve({ status: 401 });
  });
};
...
var patch = function patch(path) {
  ...
  return request('patch', path, _extends({ postData: postData }, opts));
};
...
var put = function put(path) {
  ...
  return request('put', path, _extends({ postData: postData }, opts));
};
...
exports.default = {
  ...
  unauthorizedPromise: unauthorizedPromise,
  request: request,
  get: get,
  post: post,
  patch: patch,
...
var setConfigParam = function setConfigParam(paramName, value) {
...
var setApiBaseURL = function setApiBaseURL(url) {
  _config2.default.setConfigParam('baseURL', url);
};
...
var list = function list() {
  ...
  if (!_current_user2.default.isAuthenticated()) {
    return _request2.default.unauthorizedPromise();
  };
...
var get = function get(patientID, proofID) {
  if (!_current_user2.default.isAuthenticated()) {
    return _request2.default.unauthorizedPromise();
  };
...
var save = function save(patientID, proof) {
  var opts = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

  if (proof.id) {
    return _request2.default.put(basePath(patientID) + '/' + proof.id, proof, opts);
  }
  return _request2.default.post(basePath(patientID), proof, opts);
};
